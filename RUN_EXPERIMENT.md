...
| IQ-TREE analysis models | GTR+G4, GTR, HKY+G4, HKY, and JC |
| Species-tree methods | ASTRAL-IV and support-weighted wASTRAL |
| Random seed | 6,406,001 |

This produces 60,000 IQ-TREE gene-tree analyses
(`2 ILS x 10 replicates x 3 lengths x 5 models x 200 loci`) and 300 paired
ASTRAL/wASTRAL comparisons. Expect substantial CPU time and disk usage.
...
weighting schemes. The current pipeline instead implements the fixed controlled
...
```bash
python run_project.py --check --output work/full_experiment --taxa 51 --loci 200 --replicates 10 --lengths 200 800 1600 --models gtr_g4 gtr hky_g4 hky jc --seed 6406001 --threads AUTO
```

Inspect the recorded tool versions before starting the experiment.

```bash
cat work/full_experiment/preflight.json
```

Inspect the frozen design and confirm that it matches the table above.

```bash
cat work/full_experiment/design.json
```
...
```bash
python run_project.py --output work/full_experiment --taxa 51 --loci 200 --replicates 10 --lengths 200 800 1600 --models gtr_g4 gtr hky_g4 hky jc --seed 6406001 --threads AUTO
```

For a long unattended run, use this command instead to keep it alive after the terminal closes.

```bash
nohup python run_project.py --output work/full_experiment --taxa 51 --loci 200 --replicates 10 --lengths 200 800 1600 --models gtr_g4 gtr hky_g4 hky jc --seed 6406001 --threads AUTO > work/full_experiment/run.log 2>&1 &
```

Immediately record the background process identifier after starting with `nohup`.

```bash
echo $! > work/full_experiment/run.pid
```
...
```bash
ps -p "$(cat work/full_experiment/run.pid)" -o pid,etime,%cpu,%mem,command
```

Follow the top-level output from the unattended run.

```bash
tail -f work/full_experiment/run.log
```

Count completed IQ-TREE gene trees; the final expected count is 60,000.

```bash
find work/full_experiment -type f -name '*.treefile' | wc -l
```

Count completed AliSim alignments; the final expected count is 12,000.

```bash
find work/full_experiment -type f -name 'locus_*.phy' | wc -l
```

Count unweighted species trees; the final expected count is 300.

```bash
find work/full_experiment -type f -name 'astral_unweighted.tre' | wc -l
```

Count weighted species trees; the final expected count is 300.

```bash
find work/full_experiment -type f -name 'wastral_support.tre' | wc -l
```

Show recently written external-tool logs when diagnosing progress or a failure.

```bash
find work/full_experiment -type f -name '*.log' -print0 | xargs -0 ls -lt | head -20
```
...
```bash
ls -lh work/full_experiment/results
```

Confirm that the expected seven result tables exist.

```bash
for file in stage1_branches.csv stage1_calibration.csv stage1_summary.csv stage1_replicate_summary.csv ils_verification.csv ils_summary.csv stage2_species_tree_error.csv; do test -s "work/full_experiment/results/$file" && echo "OK: $file" || echo "MISSING: $file"; done
```

Inspect the realized low/high ILS separation before interpreting treatment effects.

```bash
column -s, -t < work/full_experiment/results/ils_summary.csv
```

Inspect the Stage 1 condition-level branch-error and high-support-error summaries.

```bash
column -s, -t < work/full_experiment/results/stage1_summary.csv | less -S
```

Inspect the Stage 1 support-calibration bins.

```bash
column -s, -t < work/full_experiment/results/stage1_calibration.csv | less -S
```

Inspect the paired Stage 2 ASTRAL/wASTRAL species-tree errors.

```bash
column -s, -t < work/full_experiment/results/stage2_species_tree_error.csv | less -S
```

In Stage 2, interpret `delta = weighted error - unweighted error` as follows:
...
biological replicates, and do not report conclusions until `ils_summary.csv`
confirms that the simulated low- and high-ILS treatments are separated.
